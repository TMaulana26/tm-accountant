    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Aktivitas</p>
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Transaksi Jurnal</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white font-mono">{{ number_format($stats['transaksi'], 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dompet & Rekening</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white font-mono">{{ number_format($stats['dompet'], 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Telegram Bot</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white font-mono">{{ number_format($stats['bot'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
                    ...
                    <table class="w-full text-left text-xs">
                        <thead class="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-300 font-bold uppercase tracking-wider">
                            <tr>
                                ...
                                <th class="py-3 px-4 w-28 text-center">Detail</th>
                            </tr>
                        </thead>
                            ...
                            @foreach($activities as $act)
                                ...
                                <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition">
                                    ...
                                    <td class="py-3 px-4 text-gray-600 dark:text-gray-300">
                                        ...
                                        @else
                                            <span class="text-gray-400 italic">🤖 Sistem / Bot</span>
                                        @endif
                                    </td>
                                    <td class="py-3 px-4 text-center">
                                        @if($act->properties && $act->properties->isNotEmpty())
                                            <details class="relative inline-block text-left">
                                                <summary class="cursor-pointer px-2 py-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-[11px] font-semibold text-gray-700 dark:text-gray-300">
                                                    Lihat Data
                                                </summary>
                                                <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs" onclick="this.parentElement.removeAttribute('open')">
                                                    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl max-w-lg w-full p-5 shadow-xl text-left" onclick="event.stopPropagation()">
                                                        ...
                                                        <pre class="bg-gray-50 dark:bg-gray-950 p-3 rounded-lg text-xs font-mono overflow-x-auto text-gray-800 dark:text-gray-200 max-h-72">{{ json_encode($act->properties->toArray(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}</pre>
                                                    </div>
                                                </div>
                                            </details>
                                        @else
