<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <x-filament::section>
            <!-- Header Laporan -->
            <div class="mb-8 flex flex-col items-start justify-between gap-4 border-b border-gray-200 pb-6 dark:border-white/10 md:flex-row md:items-center">
                <div>
                    <h2 class="text-2xl font-bold tracking-tight text-gray-950 dark:text-white">BUKU BESAR</h2>
                ...
                <div class="text-left md:text-right">
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Saldo Awal: <span class="font-mono font-bold text-gray-950 dark:text-white">Rp {{ number_format($report['beginning_balance'], 2, ',', '.') }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full font-sans text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        <!-- Baris Saldo Awal -->
                        <tr class="bg-gray-50/70 text-xs font-semibold italic text-gray-700 dark:bg-white/5 dark:text-gray-300">
                            <td class="py-2.5 px-3">{{ \Carbon\Carbon::parse($report['start_date'])->translatedFormat('d/m/Y') }}</td>
                            <td class="py-2.5 px-3 text-gray-400 dark:text-gray-500">-</td>
                            <td class="py-2.5 px-3" colspan="4">Saldo Awal Sebelum Periode Ini</td>
                            <td class="py-2.5 px-3 text-right font-mono font-bold text-gray-950 dark:text-white">
                        ...
                        @forelse($report['transactions'] as $tx)
                            <tr class="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/5">
                                <td class="whitespace-nowrap py-2.5 px-3 text-gray-700 dark:text-gray-300">{{ $tx['date'] }}</td>
                                <td class="whitespace-nowrap py-2.5 px-3 font-mono font-semibold text-emerald-600 dark:text-emerald-400">{{ $tx['entry_number'] }}</td>
                                <td class="py-2.5 px-3 text-gray-900 dark:text-gray-200">{{ $tx['description'] }}</td>
                                <td class="py-2.5 px-3 text-center">
                                    <span class="inline-flex items-center rounded border px-2 py-0.5 text-xs font-medium {{ $tx['source']?->value === 'telegram' ? 'border-blue-200 bg-blue-100 text-blue-800 dark:border-blue-800 dark:bg-blue-950/80 dark:text-blue-300' : 'border-gray-200 bg-gray-100 text-gray-800 dark:border-white/10 dark:bg-white/5 dark:text-gray-300' }}">
                                        {{ $tx['source']?->getLabel() ?? 'Web' }}
                                    </span>
                                </td>
                                <td class="py-2.5 px-3 text-right font-mono font-semibold text-gray-950 dark:text-white">
                                    {{ $tx['debit'] > 0 ? number_format($tx['debit'], 2, ',', '.') : '-' }}
                                </td>
                                <td class="py-2.5 px-3 text-right font-mono font-semibold text-gray-950 dark:text-white">
                                    {{ $tx['credit'] > 0 ? number_format($tx['credit'], 2, ',', '.') : '-' }}
                                </td>
                                <td class="py-2.5 px-3 text-right font-mono font-bold {{ $tx['running_balance'] >= 0 ? 'text-gray-950 dark:text-white' : 'text-rose-600 dark:text-rose-400' }}">
                    ...
                    <tfoot>
                        <tr class="border-t-2 border-gray-900 bg-emerald-50/90 font-bold text-emerald-950 dark:border-gray-600 dark:bg-emerald-950/40 dark:text-emerald-300">
                            ...
                            <td class="py-3 px-3 text-right font-mono text-base font-black text-emerald-900 dark:text-emerald-300">
